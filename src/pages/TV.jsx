...
function TVList() {
  return (
    <div>
      <h1>TV 프로그램</h1>
      <ul>
        {tvListData.map((tvProgram) => (
          <li key={tvProgram.id}>
            <h2>{tvProgram.name}</h2>
            <p>평점: {tvProgram.vote_average}</p>
            <p>개봉일: {tvProgram.first_air_date}</p>
            <p>장르: {tvProgram.genre_ids.join(', ')}</p>
            <p>설명: {tvProgram.overview}</p>
            <img
              src={`https://image.tmdb.org/t/p/w500${tvProgram.poster_path}`}
              alt={tvProgram.name}
            />
          </li>
        ))}
